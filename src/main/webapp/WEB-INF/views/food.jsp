...
<!-- header section ends 페이지최상단종료 -->
	...
	<section class="product" id="product">
    	...
    	<c:if test="${pageMaker.page !=1}">
    		<a href='food${pageMaker.makeSearch(1)}'>&laquo;</a>
    	</c:if>
    	<c:if test="${pageMaker.prev }">
    		<a href='food${pageMaker.makeSearch(pageMaker.startPage-1)}'>&lt;</a>
    	</c:if>
    	
    	<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage}" var="idx">
    		<a href='food${pageMaker.makeSearch(idx)}' 
    	...
    	<c:if test="${pageMaker.next }">
    		<a href='food${pageMaker.makeSearch(pageMaker.endPage+1)}'>&gt;</a>
    	</c:if>
    	
    	<c:if test="${pageMaker.page != pageMaker.totalEndPage}">
    		<a href='food${pageMaker.makeSearch(pageMaker.totalEndPage)}'>&raquo;</a>
    	</c:if>
